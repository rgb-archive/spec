...
class UTXO:
    ...
    def spend(self):
        # TODO: add an optional parameter "commit_to"
    ...
    @staticmethod
    def deserialize(pb_utxo: rgb_pb2.UTXO) -> 'UTXO':
        return UTXO(pb_utxo.txid, pb_utxo.index)

    def serialize(self) -> rgb_pb2.UTXO:
        utxo = rgb_pb2.UTXO()

        if self.txid is not None:
            utxo.txid = self.txid
        utxo.index = self.index

        return utxo

    def __eq__(self, other):
        if isinstance(self, other.__class__):
            return self.txid == other.txid and \
